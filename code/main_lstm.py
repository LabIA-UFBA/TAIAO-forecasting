...
def create_dataset(series, window_size, horizon):
    ...
    for i in range(max_start):
        X.append(series[i: i + window_size])
        Y.append(series[i + window_size: i + window_size + horizon])
    # -> empilhar em NumPy antes de ir pra Torch (evita warning)
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        # opcional: ajuda a evitar o warning em CUDA
        try:
            self.lstm.flatten_parameters()
        except Exception:
            pass
        out, _ = self.lstm(x)              # x: (B, T, input_size)
        out = self.linear(out[:, -1, :])   # (B, hidden) -> (B, output_dim)
        return out
...
# ):
#     # model
#     model = LSTMModel(input_size=window_size, 
...
def get_model(
    window_size=100,    # não usamos isso como input_size!
    hidden_size=64,
    n_steps=100,
    epochs=1000,
    lr=1e-3,
    ...
    # input_size TEM que ser 1, pois você faz unsqueeze(-1)
    model = LSTMModel(input_size=1, hidden_size=hidden_size, num_layers=2, output_dim=n_steps).to(device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=lr)
    return model, criterion, optimizer
...
def exp_worflow(df: pd.DataFrame, 
    ...
    for ts_same in time_series:
        ...
        # Salvar em CSV
        forecast_df.to_csv(f"lstm/forecast_{ts_same}.csv", index=False)
